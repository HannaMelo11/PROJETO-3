...
int validar_email(const char *email) {
    ...
    // Verificar se o email possui pelo menos um caractere antes do '@'
    if (len < 3)
        return 0;
    ...
    // Verificar se há pelo menos um caractere após o último '.' após o '@'
    for (i = 0; i < len; i++) {
        if (email[i] == '@') {
        ...
        while (!valid) {
            ...
            for (int i = 0; i < *total_contatos; i++) {
                if (strcmp(lista_contatos[i].telefone, telefone) == 0) {
                    valid = 0;
                    printf("Este telefone já está em uso. Tente novamente.\n");
                    break;
                }
            }
        }
...
void modificar_contato(struct Contato lista_contatos[], int total_contatos, char telefone[]) {
    int encontrado = 0;
    ...
    for (int i = 0; i < total_contatos; i++) {
        ...
        if (strcmp(clean_telefone, telefone) == 0) {
            encontrado = 1;
            printf("Novo nome: ");
            scanf("%s", lista_contatos[i].nome); // Lê o nome diretamente sem espaços em branco

            printf("Novo sobrenome: ");
            scanf("%s", lista_contatos[i].sobrenome); // Lê o sobrenome diretamente sem espaços em branco

            // Obter e validar o novo email
            int email_valido = 0;
            while (!email_valido) {
                printf("Novo email: ");
                fgets(lista_contatos[i].email, 100, stdin);
                lista_contatos[i].email[strcspn(lista_contatos[i].email, "\n")] = 0; // Remover o caractere de nova linha do final
                email_valido = validar_email(lista_contatos[i].email);
                if (!email_valido)
                    printf("Email inválido. Tente novamente.\n");
            }

            // Obter e validar o novo telefone
            ...
            while (!valid) {
                ...
                // Verificar se contém apenas dígitos
                valid = 1;
                for (int i = 0; i < strlen(novo_telefone) - 1; i++) {
                    if (!isdigit(novo_telefone[i])) {
                        valid = 0;
                        printf("Número de telefone inválido. Tente novamente.\n");
                        break;
                    }
                ...
                // Verificar se o telefone já existe na lista de contatos
                for (int i = 0; i < total_contatos; i++) {
                    if (strcmp(lista_contatos[i].telefone, novo_telefone) == 0 && i != encontrado) { // Verifica se o telefone existe, exceto para o contato atual
                        valid = 0;
                        printf("Este telefone já está em uso. Tente novamente.\n");
                        break;
                    }
            ...
            strcpy(lista_contatos[i].telefone, novo_telefone);
